...
import { mehrdad, menu, close } from '../assets'
  ...
  return (
    ...
    max-w-7xl mx-auto'>
        ...
        className='flex items-center gap-2'
          ...
          <img src={mehrdad} alt="logo" className='w-9 h-9 object-contain' />

          <p className='text-white text-[18px] font-bold
         cursor-pointer flex'>   محمودی | &nbsp;<span className='sm:block hidden'>مهرداد</span>
         </p>
        </Link>
          ...
          {navLinks.map((link) => (
            <li key={link.id}
              className={`${active === link.title
                ? "text-white"
                : "text-secondary"
                } hover:text-white text-[16px] font-medium cursor-pointer`} onClick={() => {
                  setToggle(!toggle);
