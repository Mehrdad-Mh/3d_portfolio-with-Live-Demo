...
const FeedbacksCard = ({ testimonial, name, designation, company, image ,index }) => (
  <motion.div
    variants={fadeIn("", "spring", index * 0.5, 0.75)}
    className="bg-black-100 p-10 rounded-3xl xs:w-[320px] w-full"
  >
    <p className="text-white fomt-black text-[48px] " >
      "</p>
      ...
      <div className="mt-7 flex justify-between items-center gap-1">
        <div className="flex-1 flex flex-col">
          ...
          <p className="mt-1 text-secondary text-[12px]" >
            {designation} از {company}
          </p>
        </div>

<img
src={image}
alt={`پاسخ داده شده توسط ${name}`}
